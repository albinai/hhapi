...
при указании параметра `with_professional_roles=true` поле `specializations` перестает быть обязательным, а поле `professional_roles` становится обязательным
### Общая информация
