...
```GET /vacancies/drafts/{draft_id}/suitable_applicants```

В случае успешного выполнения запроса, будет возвращён статус `200 OK`. В теле ответа будет содержаться количество потенциальных кандидатов на черновик вакансии
...
Формат объекта `Количество потенциальных кандидатов`
...
* `403 Forbidden` — текущий пользователь не является работодателем
* `404 Not Found` — если черновик не найден или у пользователя нет прав на доступ к данным черновика
